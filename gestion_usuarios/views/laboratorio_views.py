    ...
    def get(self, request, campo="",criterio=""):
        try:
            if (len(campo)> 0 and len(criterio)> 0):
                if criterio == "id":
                    laboratorios = list(Laboratorios.objects.filter(id=campo).values())
                ...
                laboratorios = list(Laboratorios.objects.values())
                if len(laboratorios) > 0:
                    laboratorios = {'message': "Consulta exitosa", 'laboratorios': laboratorios}
                else:
                    laboratorios = {'message': "No se encontraron los datos", 'laboratorios': []} 
        ...
        try:
            ...
            if len(jd['nombre']) <= 0:
                ...
                Laboratorios.objects.create(nombre=jd['nombre'],
                                        ...
                                        disponibilidad=jd['disponibilidad'])
                mensaje_post = {'message':"Registro Exitoso."}
...
#Actualizar un registro de cargos
    def put(self, request,id):
        try:
            jd=json.loads(request.body)
            mensaje_put = list(Laboratorios.objects.filter(id=id).values())
            if len(mensaje_put) > 0:
                laboratorio_actualizar=Laboratorios.objects.get(id=id)
                if len(jd['nombre']) <= 0:
                    ...
                    laboratorio_actualizar.disponibilidad = jd['disponibilidad']
                    laboratorio_actualizar.save()
...
#Eliminar un registro de cargos
    def delete(self, request,id):
        try:
            laboratorios = list(Laboratorios.objects.filter(id=id).values())
            if len(laboratorios) > 0:
                Laboratorios.objects.filter(id=id).delete()
                mensaje_delete = {'message':"Registro Eliminado"}
            else:
                mensaje_delete = {'message':"No se encontró el registro", 'laboratorios': []}
            return JsonResponse(mensaje_delete)
        except Exception as e:
            mensaje_delete = {'message': f"Error: {e}"}
            return JsonResponse(mensaje_delete)
...
def validar_telefono_repetido(valor): 
    if (valor):
        registros = Laboratorios.objects.filter(telefono=valor)
        if len(registros) > 0:
            return True
        else:
            return False
