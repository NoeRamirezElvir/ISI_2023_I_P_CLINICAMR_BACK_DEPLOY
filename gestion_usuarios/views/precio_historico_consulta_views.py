...
class PrecioHistoricoConsultasViews(View):
    ...
    def get(self, request, campo="",criterio=""):
        if (len(campo)> 0 and len(criterio)> 0):
            if criterio == "id":
                precioHistoricos = PrecioHistoricoConsulta.objects.filter(id=campo).select_related('idTipo')
                ...
                precioHistoricos = PrecioHistoricoConsulta.objects.filter(idTipo__nombre=campo)
                if precioHistoricos is not None:
                    historicos_values = []
                    for historico in precioHistoricos:
                        historico_dict = {
            ...
            precioHistoricos = PrecioHistoricoConsulta.objects.select_related('idTipo')
            if precioHistoricos is not None:
                historicos_values = []
                for historico in precioHistoricos:
                    historico_dict = {
...
#Eliminar un registro de cargos
    def delete(self, request,id):
        precioHistorico = list(PrecioHistoricoConsulta.objects.filter(id=id).values())
        if len(precioHistorico) > 0:
            PrecioHistoricoConsulta.objects.filter(id=id).delete()
